...
public class IFGenericItemModifier extends LootModifier {
    public static final Supplier<MapCodec<IFGenericItemModifier>> CODEC = () ->
    ...
    protected @NotNull ObjectArrayList<ItemStack> doApply(LootTable lootTable, ObjectArrayList<ItemStack> generatedLoot, LootContext context) {
        ...
        generatedLoot.add(new ItemStack(item, context.getRandom().nextInt(Math.min(looting + 1, 4)) + IFConfig.GENERIC_MOB_DROP.get()));
